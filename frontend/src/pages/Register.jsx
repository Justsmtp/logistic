...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(formData).forEach(key => {
      if (key !== 'vehicleImages' && key !== 'confirmPassword') {
        submitData.append(key, formData[key]);
      }
    });

    // Add vehicle images if driver
    if (formData.role === 'driver') {
      Object.keys(formData.vehicleImages).forEach(position => {
        if (formData.vehicleImages[position]) {
          submitData.append(`vehicleImage_${position}`, formData.vehicleImages[position]);
        }
      });
    }

    // For now, we'll send JSON (you'll need to update backend to handle FormData with images)
    ...
    <div className="min-h-screen cyber-grid flex items-center justify-center px-4 py-12">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold gradient-text mb-2">Join LogiTrack</h1>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Enter Nigerian phone number (e.g., 08012345678 or +2348012345678)
                  </p>
                </div>
